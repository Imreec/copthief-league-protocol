...
| [`sparring/`](sparring/) | A practice implementation you run locally — full rulebook, no mail, simple brains. `python -m sparring.cli selfplay` ([status](sparring/README.md#status-what-is-verified-and-what-is-not)) |
...
7. Read the [sparring peer](sparring/) — a second implementation of the game layer you can run and
   copy from. `python -m sparring.cli selfplay` plays a whole series with mutual audits and writes
   the four artifacts, and it puts Hebrew on the wire so a serializer that escapes non-ASCII fails
   there rather than at a real opponent's audit.
